...
declare global {
    interface Window {
        ...
        __initSearch: () => void;
        __searchInstance?: Search;
    }
}

/// 暴露全局初始化函数，供 Swup page:view 钩子调用（有防重复 guard）
window.__initSearch = function () {
    const searchForm = document.querySelector('.search-form') as HTMLFormElement;
    if (!searchForm || searchForm.dataset.searchInit === '1') return;
    searchForm.dataset.searchInit = '1';
    ...
    window.__searchInstance = new Search({
        form: searchForm,
        input: searchInput,
...
/// 初始页面加载时（非 Swup 导航）自动初始化搜索
if (document.body && document.body.classList.contains('template-search')) {
    window.addEventListener('load', () => {
        setTimeout(() => window.__initSearch(), 0);
    });
}
